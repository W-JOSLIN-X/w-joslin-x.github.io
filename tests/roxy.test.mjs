...
import { nextTrack, readMusicPreferences, formatMusicTime } from '../src/utils/roxy-music.mjs';
const tracks = [{ id: 'one', src: '/one.mp3' }, { id: 'placeholder', src: '' }, { id: 'two', src: '/two.wav' }, { id: 'three', src: '/three.mp3' }];
...
test('single repeat only repeats on completion; shuffle excludes current and placeholders', () => {
  assert.equal(nextTrack(tracks, 'one', { mode: 'single', ended: true }), 'one');
  assert.equal(nextTrack(tracks, 'one', { mode: 'single' }), 'two');
  for (const roll of [0, .3, .8, 1]) assert.ok(['two', 'three'].includes(nextTrack(tracks, 'one', { mode: 'shuffle', roll })));
});
test('removed music tracks and corrupted preferences cannot restore invalid playback state', () => {
  const defaults = { volume: .65, mode: 'list' };
  assert.deepEqual(readMusicPreferences({ id: 'removed', time: 70, volume: 2, mode: 'bad' }, tracks, defaults), { id: 'one', time: 0, volume: 1, mode: 'list', muted: false });
  assert.equal(readMusicPreferences({ id: 'placeholder', time: 10 }, tracks, defaults).id, 'one');
  assert.equal(readMusicPreferences({ id: 'two', time: 43, muted: true }, tracks, defaults).time, 43);
  assert.equal(formatMusicTime(Infinity), '0:00');
  assert.equal(formatMusicTime(172.52), '2:52');
});
import { normalizeSlideshow, nextImage, shouldEnterScreensaver, hexToHsl, hslToHex } from '../src/utils/roxy-slideshow.mjs';
test('slideshow repairs stale selections without losing chosen order', () => {
  const ids = ['a', 'b', 'c'];
  assert.deepEqual(normalizeSlideshow({ selected: ['c', 'deleted', 'a', 'c'] }, ids).selected, ['c', 'a']);
  assert.deepEqual(normalizeSlideshow({ selected: ['deleted'] }, ids).selected, ids);
  const repaired = normalizeSlideshow({ interval: -20, idleMinutes: Infinity, auto: 'false' }, ids);
  assert.equal(repaired.interval, 3); assert.equal(repaired.idleMinutes, 5); assert.equal(repaired.auto, true);
  assert.equal(normalizeSlideshow({ auto: false, enabled: false }, ids).auto, false);
});
test('slideshow wraps selected order and shuffle never immediately repeats', () => {
  ...
  for (const roll of [0, .2, .5, .99, 1]) assert.notEqual(nextImage(['a', 'b', 'c'], 'b', 1, true, roll), 'b');
...
test('automatic screensaver cannot interrupt an article, hidden tab or active form', () => {
  const state = { article: false, open: false, visible: true, busy: false, auto: true, idleMinutes: 5, elapsed: 300000 };
  assert.equal(shouldEnterScreensaver(state), true);
  for (const override of [{ article: true }, { open: true }, { visible: false }, { busy: true }, { auto: false }, { elapsed: 299999 }]) assert.equal(shouldEnterScreensaver({ ...state, ...override }), false);
...
test('color picker preserves neutral and saturated colors across hue conversion', () => {
  for (const [hex, expected] of [['#ff0000', 0], ['#00ff00', 120], ['#0000ff', 240]]) assert.equal(hexToHsl(hex).hue, expected);
  for (const hex of ['#000000', '#ffffff', '#808080']) {
    const hsl = hexToHsl(hex); assert.equal(hsl.saturation, 0);
    const result = hslToHex(hsl.hue, hsl.saturation, hsl.lightness);
    assert.ok(Math.abs(parseInt(result.slice(1, 3), 16) - parseInt(hex.slice(1, 3), 16)) <= 1);
  }
});
import {
	monthCells,
