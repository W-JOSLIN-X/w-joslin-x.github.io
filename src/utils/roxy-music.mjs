...
export function nextTrack(
	tracks,
	currentId,
	{ direction = 1, mode = "list", ended = false, roll = Math.random() } = {},
) {
...
export function readMusicPreferences(value, tracks, defaults) {
	...
	const id = tracks.some((track) => track.id === s.id && track.src)
		? s.id
		: tracks.find((track) => track.src)?.id || null;
	return {
		id,
		time: id === s.id && Number.isFinite(s.time) && s.time >= 0 ? s.time : 0,
		volume: Number.isFinite(s.volume)
			? Math.max(0, Math.min(1, s.volume))
			: defaults.volume,
		muted: s.muted === true,
		mode: ["list", "single", "shuffle"].includes(s.mode)
			? s.mode
			: defaults.mode,
	};
}
