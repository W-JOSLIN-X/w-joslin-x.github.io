...
export const postHref = (id: string) =>
	`/posts/${slugOf(id).split("/").map(encodeURIComponent).join("/")}/`;
	...
	return (
		links.some((href) => {
			try {
				...
				return (
					["local.test", "w-joslin-x.github.io"].includes(parsed.hostname) &&
